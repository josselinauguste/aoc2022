...
class Day3Test {
    @Test
    fun `sum of the priorities of item type that appears in both compartments of each rucksack (sample)`() {
        val sum = sumPrioritiesOfItemTypesInBothCompartiments(sampleListOfContents)

        assertEquals(157, sum)
    }

    @Test
    fun `sum of the priorities of item type that appears in both compartments of each rucksack`() {
        val sum = sumPrioritiesOfItemTypesInBothCompartiments(listOfContents)

        assertEquals(8493, sum)
    }

    @Test
    fun `sum of the priorities of the badges of each three-Elf group (sample)`() {
        val sum = sumPrioritiesOfBadgesOfElfGroups(sampleListOfContents)

        assertEquals(70, sum)
    }

    @Test
    fun `sum of the priorities of the badges of each three-Elf group`() {
        val sum = sumPrioritiesOfBadgesOfElfGroups(listOfContents)

        assertEquals(2552, sum)
    }

    private val sampleListOfContents = """vJrwpWtwJgWrhcsFMMfFFhFp
jqHRNqRjqzjGDLGLrsFMfFZSrLrFZsSL
PmmdzqPrVvPwwTWBwg
wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn
ttgJtRGJQctTZtZT
CrZsJsPPZsGzwwsLwLmpwMDw"""
    private val listOfContents = """jNNBMTNzvTqhQLhQLMQL
